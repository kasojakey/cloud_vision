...
- (void) createRequest: (NSString*)imageData {
    ...
    NSString *API_KEY = @"AIzaSyBPsXZ8fyRTX9rfhw_K-6fT02HqIByrMF0";
            ...
            if (faceAnnotations != NULL) {
                ...
                for (NSDictionary *personData in faceAnnotations) {
                    for (NSString *emotion in emotions) {
                        ...
                        if (emotionValue < newValue) {
                            emotionValue = newValue;
                            emotionName = lookup;
                            emotionName = [emotionName stringByReplacingOccurrencesOfString:@"Likelihood" withString:@""];
                        }
                    }
                }
                
                if ([emotionName isEqualToString:@"sorrow"]) {
                    emotionName = @"悲傷";
                }
                else if ([emotionName isEqualToString:@"joy"]) {
                    emotionName = @"喜悅";
                }
                else if ([emotionName isEqualToString:@"surprise"]) {
                    emotionName = @"驚喜";
                }
                else if ([emotionName isEqualToString:@"anger"]) {
                    emotionName = @"憤怒";
                }
                
                [self speechText:emotionName];
